...
`WorkspaceSpec(read_only=True)` 来自 task declaration，不来自 Conductor input。read-only task 成功时不检查 target branch HEAD、不创建 staging branch、不提交 LakeFS commit，output `workspace.ref` 保持为 input `ref`。
