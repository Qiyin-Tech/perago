...
workspace task worker 的合法函数签名如下：
...
workspace-free task worker 的合法函数签名如下：
...
这些错误会在 import-time validation、`perago check` 或 worker 启动阶段失败，业务函数执行前即可暴露。
