...
| `perago start [-j N] <module_target>` | 单个 task module import path 和 worker 进程数 | 前台 supervisor 进程 | 启动前检查 Conductor TaskDef；workspace task 运行时还连接 LakeFS |

`perago check` 和 `perago extract` 是开发与发布前验证命令；`perago start` 是长运行 worker 入口。

## `perago check`

`check` 用来验证本机 runtime config、task module 导入、task contract 和 TaskDef 生成：
...
- module target 未使用 Python import path。
...
`extract` 生成 Conductor TaskDef。运行时 input payload 由 workflow 提供。它不会连接 Conductor，也不会注册 TaskDef；注册动作仍由部署流程或 Conductor 管理工具完成。
...
如果只是在开发 task body 或 schema，停在 `check` 和 `extract` 即可；不要用 `start` 代替本地校验，因为它需要真实 Conductor runtime config，workspace task 还需要真实 LakeFS runtime config。
