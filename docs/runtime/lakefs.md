...
本机 workspace 是 attempt-local 目录，不是 worker 进程的固定工作目录。每次 retry attempt 都有独立目录，避免旧 attempt 的中间文件污染新 attempt。

## Stage

task body 成功并通过第一次 attempt fence 后，Perago 会把本机 workspace 同步到 staging branch。stage 的顺序是：

1. 根据 Conductor attempt 字段生成 staging branch 名。
2. 从 `WorkspaceInput.ref` 创建 staging branch，`exist_ok=True`。
...
staging branch 是 runtime 内部状态，不进入 Conductor task output。branch 名包含 workflow、reference task、sequence、iteration、task id 和 retry count：

```text
perago-staging-<workflow>-<reference>-seq-<seq>-iteration-<iteration>-task-id-<task_id>-retry-<retry_count>
```

stage 成功后，runtime 只把 staging branch 和 staging commit id 保存在 `StagedWorkspace` 中，供后续 publish 和 cleanup 使用。
