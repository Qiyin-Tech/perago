...
2. 从 `PERAGO_WORKSPACE_ROOT` 创建 attempt-local workspace。
...
| 动作 | 说明 |
| --- | --- |
| create staging branch | 从 input `workspace.ref` 创建 attempt-scoped staging branch。 |
...
staging branch 名由 Conductor attempt 字段生成，包含 workflow、reference task、sequence、iteration、task id 和 retry count。它是内部 runtime 状态，不进入 Conductor input/output，也不要求任务作者手动管理。
...
如果 LakeFS merge 已经成功，cleanup 失败不会回滚目标 branch。Perago 的恢复模型是通过 metadata 识别已发生的 publication，而不是反向修改已发布 commit。
...
- 不要在 merge timeout 或连接错误后盲目重试同一个 merge；先检查目标 branch commit metadata 是否已有匹配的 `perago.logical_task_key`、`perago.task_id` 和 `perago.staging_commit`。
- 如果 runtime 无法根据 metadata 分类 LakeFS 状态，应 fail closed，让当前 workflow 失败，再从当前 protected branch head 发起新的工作流。
