...
1. 从 input `workspace.ref` 物化本地 attempt workspace，完成业务执行、result 校验和本地输出检查；到这一步为止不能创建 staging branch，也不能写入远端 LakeFS object。
2. 重新读取当前 Conductor attempt，确认它仍是 `IN_PROGRESS`，且 `workflow_instance_id`、`task_id`、`retry_count` 与已领取的 attempt 一致；这次 pre-stage attempt fence 必须发生在任何 LakeFS staging side effect 之前。
3. 从 input `workspace.ref` 创建本次 execution 专属 staging branch。
4. 只在 contract 允许的 prefix 内写入、删除或更新 staging branch 上的 object。
5. 提交 staging branch，并写入 `perago.phase=try` metadata。
6. 再次重新读取当前 Conductor attempt，并执行同一组 attempt fence 检查。
7. 读取 target branch 当前 head，并沿 first-parent history 回溯到 input `workspace.ref`。
8. 仅当 target head 等于 input ref，或这段 history 全部属于同一 `perago.logical_task_key` 时继续发布；否则 fail closed。
9. 将 staging branch squash merge 到 target branch，并写入 `perago.phase=confirm` metadata。
10. 用 merge commit id 生成成功 output 的 `workspace.ref`。
11. 尝试清理 staging branch；cleanup 失败只能记录日志，不能覆盖原始结果。
