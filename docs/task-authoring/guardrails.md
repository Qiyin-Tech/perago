...
- optional: `pre` 和 `post` 都可省略；省略表示该阶段没有文件形状检查。
...
非法形状会在 module import、`perago check` 或 `WorkspaceSpec(...)` 构造时失败：
...
Guardrail 是 Perago runtime metadata，不是 Conductor TaskDef schema 的一部分。生成的 TaskDef 仍只描述 `workspace`、`params` 和 `result`。
