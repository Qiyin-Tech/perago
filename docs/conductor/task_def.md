...
This should not be confused with [*Task Configurations*](https://conductor-oss.github.io/conductor/documentation/configuration/workflowdef/index.html#task-configurations) which are part of the Workflow Definition, and are iterated in the `tasks` property in the definition.
...
- `TIME_OUT_WF`: Workflow is marked as TIMED\_OUT and terminated. This is the default value.
- `ALERT_ONLY`: Registers a counter (task\_timeout)
