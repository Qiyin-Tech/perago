...
`params` schema 来自任务函数第二个参数或唯一参数的 Pydantic 类型注解。Perago 会 inline `$ref`，并删除 Pydantic `title` 和从 `BaseModel` class docstring 自动生成的 object-level `description`。Input 顶层使用 `additionalProperties: true`；`params` 默认允许额外字段，`@task(strict_params=True)` 时把 params 及其嵌套 Pydantic object 设置为 `additionalProperties: false`。Output 和 workspace schema 保持 strict。
