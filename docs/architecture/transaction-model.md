...
| 未确认写入不污染目标 branch | 所有写入先进入 attempt-scoped staging branch。 |
...
| 阶段 | Runtime 行为 | 成功产物 |
| --- | --- | --- |
| Try | 从 input `workspace.ref` 创建 staging branch，把本机 workspace 的 `WorkspaceSpec.prefix` 投影同步进去，并提交 staging commit。 | 带 `perago.phase=try` metadata 的 staging commit。 |
...
如果 worker 在 LakeFS merge 成功后、Conductor completion 前死亡，后续 retry 可能重新执行任务。Perago 用这些 metadata 区分“同一 logical task 的前一次发布”和“无关 branch advancement”。后续 retry 发布新的线性 commit，并通过 `perago.supersedes` 记录它覆盖的前序 commit。
...
| merge 已成功但 worker 死亡 | 依赖 commit metadata 和后续 retry 分类；无法分类时 fail closed。 |

Fail closed 的恢复方式不是在原 attempt 内猜测补偿动作，而是让当前 workflow 失败，并从当前 protected branch head 发起新的工作流。
...
- LakeFS Community hook 可以作为未来 hard fence 候选，但必须先用部署版本的集成测试证明其语义；MVP 不依赖它。
