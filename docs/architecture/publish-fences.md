...
Perago 的 publish fence 解决的是 workspace task 在发布前能不能继续推进 target branch 的问题。正式操作协议见 [LakeFS 发布协议](../lakefs-publication-protocol.md)。
...
| Publish fence | LakeFS target branch HEAD 状态 | staging branch publish 前 | `FAILED` |
...
| Target branch 状态 | 行为 |
| --- | --- |
| current head 等于 input `workspace.ref` | 允许 merge staging branch。 |
| current head 的直接 parent 等于 input `workspace.ref` | 允许 replacement publish，把 target branch hard-reset / relocate 到本次 staged commit。 |
...
| 允许 retry 覆盖一个 abandoned publication，并保持可见历史为 `input_ref -> staged_commit`。 | 证明 worker 崩溃窗口内不会发生重复 publication。 |
| 对无法解释的 branch advancement fail closed。 | 在 Conductor completion 与 LakeFS publish 之间提供 XA 事务。 |

最重要的崩溃窗口是 LakeFS publish 已成功、但 worker 在 Conductor completion 前死亡。后续 retry 从 Conductor 角度仍然可能有效；如果 target branch 当前 head 是 input ref 的直接子提交，runtime 可以用 replacement publish 把可见历史替换为本次 staged commit。
